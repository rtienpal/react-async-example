...
const ConcurrentApp = () => {
  ...
  const timeoutRefs = useRef<{ [key: string]: number | undefined }>({});
  ...
  const handleClick = async (shape: Shape) => {
    const newShape = { ...shape, id: `${shape.id}-${Date.now()}` };
    setQueue((prevQueue) => [...prevQueue, newShape]);

    try {
      const response = await fetch(`http://localhost:3001/${shape.id}`);
      if (!response.ok) {
        throw new Error(`Failed to fetch shape data: ${response.status}`);
      }
      ...
      setQueue((prevQueue) =>
        prevQueue.filter((item) => item.id !== newShape.id)
      );
    } catch (error) {
